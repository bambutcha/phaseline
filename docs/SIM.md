...
`firesAt = 0.32 * duration` сек по умолчанию, плюс джиттер из сида в диапазоне ±10s, но детерминированный.

## Порог победы

`colony_win_threshold = 100`  
Pyrrhic: colony ≥ 100 и earth < 40.  
Длительность смены: 180 секунд. Тень непроходима: в погасший гекс войти нельзя, накрытие ровера = stranded. Пины забора уникальны. Смена не кончается от набранных 100 очков. 100 колонии достижимы (потолок сдаваемых заказов ≥ 100, dual-rover solver в тестах).
